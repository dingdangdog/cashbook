      ...
      <div class="w-full max-w-[26rem]">
        <div class="rounded-xl shadow-xl p-4 md:p-6 border bg-surface border-border">
          <form @submit.prevent="login" class="space-y-4">
            <!-- 用户名输入 -->
            <div>
              <input id="username" v-model="loginParam.username" type="text" name="username" autocomplete="username"
                :class="[
              ...
              <p v-if="loginFormErrors.username" class="mt-1 text-sm text-red-500">
            ...
            <div>
              <div class="relative">
                <input id="password" v-model="loginParam.password" :type="lookLoginPS ? 'text' : 'password'"
                  name="password" autocomplete="current-password" :class="[
                ...
                <button type="button" @click="lookLoginPS = !lookLoginPS"
              ...
              <p v-if="loginFormErrors.password" class="mt-1 text-sm text-red-500">
        ...
        <div class="p-2">
          ...
          <form @submit.prevent="register" class="space-y-4">
            ...
            <div>
              ...
              <input id="name" v-model="registerParam.name" type="text" name="name"
            ...
            <!-- 账号 -->
            <div>
              <label for="register-username" class="block text-sm font-medium text-foreground/80 mb-2">
                账号 *
              </label>
              <input id="register-username" v-model="registerParam.username" type="text" name="username" :class="[
              ...
              <p v-if="registerFormErrors.username" class="mt-1 text-sm text-red-500">
                {{ registerFormErrors.username }}
              </p>
            </div>
            ...
            <!-- 密码 -->
            <div>
              <label for="register-password" class="block text-sm font-medium text-foreground/80 mb-2">
                密码 *
              </label>
              <div class="relative">
                <input id="register-password" v-model="registerParam.password"
                  :type="lookRegisterPS ? 'text' : 'password'" name="password" :class="[
                ...
                <button type="button" @click="lookRegisterPS = !lookRegisterPS"
              ...
              <p v-if="registerFormErrors.password" class="mt-1 text-sm text-red-500">
                {{ registerFormErrors.password }}
              </p>
            </div>
            ...
            <!-- 确认密码 -->
            <div>
              <label for="confirm-password" class="block text-sm font-medium text-foreground/80 mb-2">
                确认密码 *
              </label>
              <div class="relative">
                <input id="confirm-password" v-model="registerParam.againPassword"
                  :type="lookRegisterAPS ? 'text' : 'password'" name="confirmPassword" :class="[
                    'w-full px-4 py-3 pr-12 border rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors duration-200 bg-background text-foreground placeholder-foreground/40',
                    registerFormErrors.againPassword
                      ? 'border-red-500 bg-red-900/10'
                      : 'border-border',
                  ]" placeholder="请确认密码" />
                <button type="button" @click="lookRegisterAPS = !lookRegisterAPS"
                  class="absolute right-3 top-1/2 -translate-y-1/2 text-foreground/40 hover:text-foreground/70 transition-colors">
                  <EyeIcon v-if="!lookRegisterAPS" class="w-5 h-5" />
                  <EyeSlashIcon v-else class="w-5 h-5" />
                </button>
              </div>
              <p v-if="registerFormErrors.againPassword" class="mt-1 text-sm text-red-500">
                {{ registerFormErrors.againPassword }}
              </p>
            </div>

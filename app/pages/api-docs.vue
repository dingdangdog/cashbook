...
<template>
  ...
  <div class="fixed top-2 right-2 z-10">
    <button @click="toggleTheme()"
      class="p-2 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg bg-surface border border-border text-foreground hover:bg-surface-muted"
      title="切换主题">
      <SunIcon v-if="isDark" class="w-5 h-5 text-foreground" />
      <MoonIcon v-else class="w-5 h-5 text-foreground" />
    </button>
  </div>
  <div class="p-6 max-w-6xl mx-auto text-foreground">
    <!-- 顶部信息 -->
    <div class="mb-10" v-if="spec.info">
      <h1 class="text-3xl font-bold">{{ spec.info.title }}</h1>
      <p class="text-muted">版本: {{ spec.info.version }}</p>
    ...
    <div v-for="[tag, apis] in groupedPaths" :key="tag" class="mb-8">
      ...
      <div @click="toggleTag(tag)"
        class="flex items-center justify-between cursor-pointer bg-surface-muted hover:bg-surface-muted/80 p-4 rounded-lg border border-border transition-colors">
        <h2 class="text-2xl font-bold text-accent-600">{{ tag }}</h2>
        <div class="flex items-center gap-2">
          <span class="text-sm text-muted">{{ apis.length }} 个接口</span>
          <svg :class="{ 'rotate-180': expandedTags.has(tag) }"
            class="w-5 h-5 text-muted transition-transform duration-200" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
      ...
      <div v-show="expandedTags.has(tag)" class="mt-4 space-y-4">
        <div v-for="api in apis" :key="api.path + api.method"
          class="border border-border rounded-lg p-4 bg-surface shadow-sm">
          <div class="flex items-center gap-2">
            <span :class="getMethodColor(api.method)"
          ...
          <details class="mt-2">
            <summary class="cursor-pointer text-primary-600 underline text-sm">
              查看接口详情
            </summary>
            ...
            <div class="mt-2 text-foreground">
              ...
              <div class="mt-3" v-if="api.details.requestBody">
                <div v-if="api.details.requestBody?.content?.['application/json']">
                  <h3 class="font-semibold text-sm text-foreground mb-1">
                    请求体:
                  </h3>
                  <pre class="bg-surface-muted p-2 rounded text-sm overflow-x-auto">{{
                    JSON.stringify(
    ...
    <div v-if="spec.components?.schemas" class="mt-20">
      <h2 class="text-2xl font-bold text-primary-600 mb-4">实体定义 (Schemas)</h2>
      <div v-for="(schema, name) in spec.components.schemas" :key="name" class="mb-4">
        <details>
          <summary class="cursor-pointer text-primary-600 underline text-sm">
            {{ name }}
          </summary>
          <pre class="bg-surface-muted p-3 mt-1 rounded text-sm overflow-x-auto">{{ JSON.stringify(schema, null, 2) }}
          </pre>
        </details>
      </div>
    </div>

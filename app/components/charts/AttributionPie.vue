<template>
  <div class="chart-common-container">
    <!-- 时间筛选侧边栏 -->
    <div v-if="searchDrawer" class="fixed inset-0 z-50 flex justify-end" @click="searchDrawer = false">
      <div class="w-80 h-full bg-surface shadow-xl border-l border-border p-2 md:p-4 overflow-y-auto" @click.stop>
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-foreground">
            时间筛选
          </h3>
          <button @click="searchDrawer = false" class="p-2 text-muted hover:text-foreground transition-colors">
        ...
        <div class="space-y-4">
          <UiDatePicker v-model="startDay" label="开始日期" placeholder="请选择开始日期" @change="changeStartDay" />

          <UiDatePicker v-model="endDay" label="结束日期" placeholder="请选择结束日期" @change="changeEndDay" />
        </div>
      </div>
    </div>

    <div class="flex flex-col md:flex-row md:justify-between items-center w-full border-b border-border md:h-16 mb-4">
      <div>
        <h4 class="hidden md:flex text-lg font-semibold text-foreground my-2">
          {{ title }}【{{ chartParam.flowType }}】
        </h4>
      </div>

      <div class="flex space-x-2 pb-2 items-center">
        <button @click="searchDrawer = true"
        ...
        <div class="min-w-32">
          <select v-model="chartParam.flowType"
            ...
            <option v-for="type in FlowTypes" :key="type.value" :value="type.value">
              {{ type.title }}
            </option>
          </select>
        </div>
  ...
  <div v-if="showFlowTable" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50"
    ...
    <div class="w-full max-w-6xl max-h-[90vh] bg-surface rounded-lg shadow-xl overflow-hidden" @click.stop>

...
export const useAppTheme = () => {
  ...
  const applyTheme = (dark: boolean) => {
    if (typeof window !== 'undefined') {
      ...
      console.log('[Theme] HTML classes before:', htmlElement.classList.toString());
      ...
      console.log('[Theme] HTML classes after:', htmlElement.classList.toString());
    }
  };
  ...
  const initTheme = () => {
    ...
    isInitialized = true;
    console.log('[Theme] Initialization complete, globalIsDark:', globalIsDark.value);
  };

  // 切换主题
  const toggleTheme = () => {
    console.log('[Theme] toggleTheme called, current isDark:', globalIsDark.value);
